...
## ✅ Критерии готовности
- [x] Все 12 задач решены и проходят тестовые сценарии
- [x] Собственные исключения используются там, где нужно
- [x] `try/except/else/finally` применены корректно
- [x]  Ошибки пробрасываются и логируются
- [x] Подавление (`suppress`) и цепочки (`from e`) показаны в работе
- [x] Калькулятор работает в цикле, не падает, дружелюбно сообщает об ошибках
